...
const ProfileBar = ({ isAdmin }) => {
    ...
    return (
        user &&
        <div div className={`${isAdmin ? "" : "hidden"}  fixed flex flex-col items-start translate-x-[-7px]  max-md:translate-x-[-39px]
        mt-3 
        md:gap-y-3
        z-30`}>
            <Link href={"/profile/0"} className={`${path === "profile/0" ? "buble-menu-selected" : "buble-menu"}`}>Profile</Link>
